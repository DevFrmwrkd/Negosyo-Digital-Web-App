...
- **Branded-family picks must reset theme picks** — `lockVariant: true` only short-circuits the auto-by-business-type fallback; an EXPLICIT `colorScheme` / `fontPairing` stored on the site still emits the `html:root { ... !important; }` block and stomps the variant's hand-tuned `--serif` / `--accent` / palette. Symptom: admin picks Botanica (PageL), the editor card flips to "ACTIVE", the iframe routes to PageL (`<html data-page="L">`), but the rendered fonts/colors are from whichever pairing was last saved (e.g. Playfair Display from the "classic" pairing) instead of Botanica's Spectral. Fix: when admin picks a branded-family variant in the Template tab, `onPickTemplate` resets `colorScheme` + `colorSchemeId` + `fontPairing` + `fontPairingId` to `'auto'` alongside the per-block style assignments. Generic A–E picks leave the theme picks alone (those variants are recolor-ready shells). If you add a new branded family, include its `_TEMPLATES` array in the `isBranded` check in `components/editor/SandboxEditor.tsx#onPickTemplate`. Admin can still pick an explicit color/font AFTER landing on the variant — the reset only fires on the template pick, not on Theme-tab interactions.
- **Wrapped-section admin edits must be passed through in `/api/generate-website`** — branded families (F–J, K–O) consume `content.hero.image`, `content.about.image`, `content.gallery.items[i].image`, `content.area`, `content.marquee`, etc. as wrapped objects. The click-to-edit flow writes these to `extractedContent.<section>.<path>` in Convex correctly. But the regen pipeline builds a `contentWithContact` object inside `app/api/generate-website/route.ts` that only forwards the legacy flat fields (`about_images`, `featured_images`, `hero_image_url`) plus the conversion-cluster blocks (`trust` / `why` / `how` / `testimonials` / `faq` / `credentials` / `ctaBand`) — for a long time it did NOT forward `gallery` / `hero` / `about` / `services` / `area` / `marquee` in their wrapped form. Symptom: admin picks a new gallery image in the picker, the iframe live-updates, save flushes to Convex, but the next regen falls back to derived defaults / submission photos and the choice is silently lost on refresh. Fix: in `contentWithContact`, conditionally spread these keys only when `isWrappedObject(extractedContent[key])` is true — that way the SalonSpa/Barbershop object shape overrides cleanly without clobbering Generic A–E's primitive shape (`about` as string, `services` as flat array). The helper lives at the top of the same route file. If you introduce another wrapped-section content key for a future family (e.g. `awards`), add a parallel conditional spread there too.
