...
> **v1.2 Update:** Auto-funding via API has been removed. Transfers are now created automatically in Wise but require **manual approval/funding by the admin in the Wise dashboard**. This avoids Wise SCA (Strong Customer Authentication) friction and gives admins explicit control over each payout.
...
│  ├─ Set to "processing" after Wise transfer is created        │
│  ├─ Send notification                                         │
│  └─ Log audit trail                                           │
└─────────────────────────────────────────────────────────────────┘
                             ↓
...
│  SERVICE         │                  │  SYSTEM          │
│                  │                  │                  │
│ 1. Create        │                  │ Send to user:    │
│    recipient     │                  │ "Withdrawal      │
│    (email)       │                  │  submitted —     │
│ 2. Create quote  │                  │  awaiting        │
│    (PHP→PHP)     │                  │  approval"       │
│ 3. Create        │                  │                  │
│    transfer      │                  │ Email + Push     │
│ 4. Save Wise IDs │                  │                  │
│    → status:     │                  │ Later (webhook): │
...
│ • Transfer appears in admin's Wise dashboard as         │
│   "Awaiting your approval" — needs manual funding       │
│ • Admin clicks "Fund" → Wise debits PHP balance         │
│ • Wise sends money to user's Wise email                 │
│ • Webhook fires → app marks withdrawal as completed     │
└──────────────────────────────────────────────────────────┘
        ↓
┌─────────────────────────────────────────────────────────┐
│              AUDIT LOGS                                │
...
Response:
{
  ...
  "message": "Withdrawal submitted! It will be reviewed by an admin and you will be notified once processed."
}
```
...
#### **Stage 4: Wise Transfer Creation (Background Process)**
```
wise.initiateTransfer() action:

1. Get Wise API service config
   └─ WISE_API_TOKEN + WISE_PROFILE_ID from environment
   └─ WISE_SANDBOX flag (false = production)

2. Create email recipient
   └─ POST /v1/accounts
   └─ Type: "email"
   └─ Email: creator@email.com
   └─ Account holder name: Creator's full name
   └─ Returns: recipient.id

3. Create quote (PHP → PHP, 1:1 rate)
   └─ POST /v1/quotes
...
4. Create transfer
   ...
   └─ quote: quote.id
   └─ reference: withdrawal._id (for tracking)
   └─ Returns: transfer.id

5. Save Wise IDs to withdrawal record
   └─ setWiseTransferIds() stores wiseTransferId and wiseRecipientId
   └─ status becomes `processing`

6. Funding step (skipped intentionally)
   └─ NO automatic API funding call
   └─ Transfer is now visible in admin's Wise dashboard
   └─ Status remains `processing` until admin manually funds it
```

#### **Stage 4b: Admin Manual Approval (Wise Dashboard)**
```
Admin opens Wise dashboard → wise.com/login

1. Navigate to "Payments" or "Transactions"
   └─ Sees pending transfer for ₱300 to creator@email.com

2. Verify the transfer
   └─ Cross-reference creator name, amount, reference code
   └─ Confirm the creator's withdrawal request is legitimate

3. Click "Fund" / "Approve"
   └─ Wise debits the platform's PHP balance
   └─ Transfer enters Wise's outgoing payment pipeline

4. Wise processes the transfer
   └─ Sends to creator's Wise account
   └─ Usually completes within minutes for Wise email transfers
```

#### **Stage 4c: Webhook Updates Status (Automatic)**
```
Wise fires webhook → https://<convex-deployment>.convex.site/wise-webhook

1. Convex http.ts handler receives event
   └─ Rate limited (60/min)
   └─ Signature verified (RSA-SHA256)

2. Maps Wise state → withdrawal status
   ┌──────────────────────┬─────────────┐
   │ Wise state           │ App status  │
   ├──────────────────────┼─────────────┤
...
3. updateByTransactionRef() runs
   └─ On completed: increments creator.totalWithdrawn,
     sends "Withdrawal Completed!" push notification
   └─ On failed: restores creator.balance,
...
```
Timeline:
- Immediately: balance is deducted in-app and withdrawal enters `processing`
- Within seconds: transfer appears in admin's Wise dashboard
- Awaiting admin: admin reviews and approves/funds in Wise dashboard
- Within minutes of approval: Wise processes the email transfer
- Webhook fires: app marks withdrawal as `completed`
- Creator receives money in their Wise account

User notification:
Email/Push: "Withdrawal submitted — awaiting approval"
Then: "Withdrawal completed!" when webhook confirms

Mobile app auto-updates:
- Balance reflects new amount (₱1,200) immediately
- Withdrawal shows `processing` until admin funds
- Webhook updates to `completed`/`failed`
```
...
2. Validate balance
3. Check if method is 'wise_email'
...
#### **3. Convex Action: `wise.initiateTransfer()`**
```typescript
// Location: convex/wise.ts
...
Process:
1. Create Wise recipient (email)
2. Create Wise quote (PHP→PHP)
3. Create Wise transfer (linked to recipient + quote)
4. Save Wise IDs to withdrawal record (status → processing)
5. STOP — no auto-funding call
   └─ Transfer is now visible in admin's Wise dashboard
   └─ Admin must manually fund/approve it from there
6. If any of steps 1-3 fail:
   ├─ markFailed()
   └─ Restore creator balance

Wise webhook handles final state (after admin approves):
   ├─ outgoing_payment_sent → completed
   ├─ cancelled/funds_refunded/bounced_back → failed
   └─ updateByTransactionRef()

Supporting Wise service handles:
   ├─ Create email recipient
   ├─ Create quote
   ├─ Create transfer
   └─ fundTransfer() returns "WAITING_FOR_APPROVAL" stub
       (no actual API funding call is made)

Error Handling:
- Wise API errors during recipient/quote/transfer creation → mark failed
- Webhook mismatch/environment mismatch → stays processing until fixed
- Admin never funds the transfer → stays processing indefinitely
  (admin should periodically review pending transfers in Wise dashboard)
```

#### **4. Wise API Service: `services/wise.ts`**
```typescript
// Location: services/wise.ts
...
2. Get profile ID from WISE_PROFILE_ID env var
3. Call Wise API to create email recipient
   └─ POST /v1/accounts (type: "email")
4. Call Wise API to create quote
   └─ POST /v1/quotes (PHP → PHP, 1:1)
5. Call Wise API to create transfer
   └─ POST /v1/transfers (links recipient + quote)
6. fundTransfer() returns stub WAITING_FOR_APPROVAL
   └─ NO API call to /v3/profiles/.../payments
   └─ Admin must approve/fund manually in Wise dashboard
7. Webhook delivers final status back to Convex once admin funds
```
...
├─ creatorId: Id<"creators">
├─ amount: number
├─ payoutMethod: enum [wise_email, bank_transfer]
├─ accountDetails: string
├─ wiseEmail?: string
├─ accountHolderName: string
├─ status: enum [pending, processing, completed, failed]
├─ failureReason?: string
├─ transactionRef?: string     // Wise transfer ID for webhook mapping
├─ wiseTransferId?: string
├─ wiseRecipientId?: string
├─ createdAt: timestamp
├─ processedAt: timestamp
└─ indexes: by_creator, by_status, by_transactionRef

AUDIT_LOGS TABLE:
Action types updated:
├─ 'payment_sent'
└─ plus standard audit metadata (amount, creatorId, transactionRef)
...
|---------|------|---------|
| 1.1 | Apr 10, 2026 | Updated to processing-first flow with manual Wise approval fallback and webhook auto-completion |
...
1. **Withdrawal shows "Processing" indefinitely**
   - **Most likely cause:** Admin hasn't funded the transfer yet in the Wise dashboard
   - Action: Open Wise dashboard → Payments/Transactions → find transfer by `wiseTransferId` → click Fund
   - Check: Wise webhook URL points to the same Convex deployment as the mobile app
   - Check: `WISE_WEBHOOK_PUBLIC_KEY` is configured for that deployment

2. **"Insufficient balance" error (in app)**
   - Check: Creator's wallet balance ≥ withdrawal amount?
   - Action: Wait for new earnings to arrive
   - Contact support if earnings missing

3. **Wise transfer creation failed (steps 1-3)**
   - Check: Recipient email is valid?
   - Check: `WISE_API_TOKEN` and `WISE_PROFILE_ID` are correct in Convex env
   - Check: Token has full access permission (not read-only)
   - Action: Withdrawal is auto-marked failed and balance is restored — creator can retry

4. **Admin funded transfer in Wise but app still shows processing**
   - Check: Webhook URL is set in Wise dashboard → Settings → Webhooks
   - Check: Webhook is subscribed to "Transfer update events"
   - Check: Convex logs for `[Wise Webhook]` entries
   - Action: Admin can manually mark withdrawal completed via admin dashboard if webhook is misconfigured

5. **Balance not updating on mobile app**
